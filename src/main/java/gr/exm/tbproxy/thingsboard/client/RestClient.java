package gr.exm.tbproxy.thingsboard.client;

import com.fasterxml.jackson.databind.JsonNode;
import com.fasterxml.jackson.databind.ObjectMapper;
import com.fasterxml.jackson.databind.node.ObjectNode;
import gr.exm.tbproxy.thingsboard.client.utils.RestJsonConverter;
...
public class RestClient implements ClientHttpRequestInterceptor, Closeable {
    ...
    public void refreshToken() {
        ...
        setTokenInfo(Objects.requireNonNull(tokenInfo.getBody()));
    }

    public JsonNode refreshTokenTransparently(String refreshToken) {
        Map<String, String> refreshTokenRequest = new HashMap<>();
        refreshTokenRequest.put("refreshToken", refreshToken);
        ResponseEntity<JsonNode> tokenInfo = restTemplate.postForEntity(baseURL + "/api/auth/token", refreshTokenRequest, JsonNode.class);
    ...
    public JsonNode loginTransparently(String username, String password) {
        Map<String, String> loginRequest = new HashMap<>();
        loginRequest.put("username", username);
        loginRequest.put("password", password);
        ResponseEntity<JsonNode> tokenInfo = restTemplate.postForEntity(baseURL + "/api/auth/login", loginRequest, JsonNode.class);
    ...
    public PageData<Asset> getCustomerAssets(CustomerId customerId, String assetType, PageLink pageLink) {
        Map<String, String> params = new HashMap<>();
        params.put("customerId", customerId.getId().toString());
        params.put("type", assetType);
        addPageLinkToParam(params, pageLink);
    ...
    public Optional<Asset> findAsset(String name) {
        ...
        try {
            ...
            return Optional.of(Objects.requireNonNull(assetEntity.getBody()));
        } catch (HttpClientErrorException exception) {
            if (exception.getStatusCode() == HttpStatus.NOT_FOUND) {
                return Optional.empty();
            } else {
    ...
    public Optional<Customer> findCustomer(String title) {
        ...
        try {
            ...
            return Optional.of(Objects.requireNonNull(customerEntity.getBody()));
        } catch (HttpClientErrorException exception) {
            if (exception.getStatusCode() == HttpStatus.NOT_FOUND) {
                return Optional.empty();
            } else {
    ...
    public List<DashboardInfo> findTenantDashboards() {
        try {
            ...
            return Objects.requireNonNull(dashboards.getBody()).getData();
        } catch (HttpClientErrorException exception) {
            if (exception.getStatusCode() == HttpStatus.NOT_FOUND) {
    ...
    public Map<String, List<JsonNode>> getTimeseries(EntityId entityId, List<String> keys, Long interval, Aggregation agg, TimePageLink pageLink) {
        return getTimeseries(entityId, keys, interval, agg, pageLink, true);
    }

    public Map<String, List<JsonNode>> getTimeseries(EntityId entityId, List<String> keys, Long interval, Aggregation agg, TimePageLink pageLink, boolean useStrictDataTypes) {
        Map<String, String> params = new HashMap<>();
        params.put("entityType", entityId.getEntityType().name());
        params.put("entityId", entityId.getId().toString());
    ...
    public boolean saveEntityTelemetry(EntityId entityId, JsonNode request) {
        return restTemplate
                .postForEntity(
    ...
    public Optional<JsonNode> getAttributes(String accessToken, String clientKeys, String sharedKeys) {
        ...
        try {
            ...
            return Optional.of(Objects.requireNonNull(telemetryEntity.getBody()));
        } catch (HttpClientErrorException exception) {
            if (exception.getStatusCode() == HttpStatus.NOT_FOUND) {
                return Optional.empty();
            } else {
    ...
    private String getUrlParams(PageLink pageLink) {
        String urlParams = "pageSize={pageSize}&page={page}";
        if (!hasText(pageLink.getTextSearch())) {
